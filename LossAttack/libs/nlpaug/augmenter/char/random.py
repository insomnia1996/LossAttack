...
from LossAttack.libs.nlpaug import CharAugmenter
from LossAttack.libs.nlpaug import Action, Method
...
class RandomCharAug(CharAugmenter):
    ...
    >>> from LossAttack.libs import nlpaug as nac
    >>> aug = nac.QwertyAug()
    """
    ...
    def swap(self, data,aug_idxes=None):
        results = []
        tokens = self.tokenizer(data)
        if aug_idxes is None:
            aug_idxes = self._get_aug_idxes(tokens, self.aug_word_p, Method.WORD)
        if aug_idxes is None:
            return data

        for token_i, token in enumerate(tokens):
            if token_i not in aug_idxes:
                results.append(token)
                continue
