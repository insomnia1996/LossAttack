...
from LossAttack.models.neuronlp2.nn import TreeCRF, VarGRU, VarRNN, VarLSTM, VarFastLSTM
from LossAttack.models.neuronlp2.nn import BiAffine, BiLinear, CharCNN
...
class StackPtrNet(nn.Module):
    def __init__(self, config, rnn_mode='FastLSTM', 
                 word_embedding=None, p_in=0.33, p_out=0.33, p_rnn=(0.33, 0.33),
                 pos=True, prior_order='inside_out', grandPar=False, sibling=False, activation='elu'):

        super(StackPtrNet, self).__init__()
        self.config = config
        self.word_embed = nn.Embedding(word_embedding.size(0), word_embedding.size(1), padding_idx=config.pad_index)#.from_pretrained(word_embedding, padding_idx=config.pad_index)
        self.pos_embed = nn.Embedding(config.n_tags, config.n_tag_embed, padding_idx=config.pad_index) if pos else None
        self.char_embed = nn.Embedding(config.n_chars, config.n_char_embed, padding_idx=config.pad_index)
        self.char_cnn = CharCNN(2, config.n_char_embed, config.n_char_out, hidden_channels=config.n_char_out * 4, activation=activation)

        self.dropout_in = nn.Dropout2d(p=p_in)
        self.dropout_out = nn.Dropout2d(p=p_out)
        self.num_labels = config.n_rels
        ...
        dim_enc = config.n_embed + config.n_char_out
        if pos:
            dim_enc += config.n_tag_embed

        self.encoder_layers = config.n_lstm_layers
        hidden_size = config.n_lstm_hidden
        self.encoder = RNN_ENCODER(dim_enc, hidden_size, num_layers=config.n_lstm_layers, batch_first=True, bidirectional=True, dropout=p_rnn)

        dim_dec = hidden_size // 2
        self.src_dense = nn.Linear(2 * hidden_size, dim_dec)
        self.decoder_layers = config.n_lstm_layers
        self.decoder = RNN_DECODER(dim_dec, hidden_size, num_layers=config.n_lstm_layers, batch_first=True, bidirectional=False, dropout=p_rnn)

        self.hx_dense = nn.Linear(2 * hidden_size, hidden_size)

        arc_space = config.n_mlp_arc
        type_space = config.n_mlp_rel
    ...
    def loss(self, input_word, input_char, input_pos, heads, type_ids, mask_e=None, mask_d=None):
        stacked_heads, children, siblings, stacked_types, skip_connect=[],[],[],[],[]
        for idx in range(heads.shape[0]): 
            stacked_head, child, sibling, stacked_type, skip_con = _generate_stack_inputs(heads[idx], type_ids[idx], 'inside_out')
            stacked_heads.append(stacked_head)
            children.append(child) 
            siblings.append(sibling)
            stacked_types.append(stacked_type)
            skip_connect.append(skip_con)
        ...
        skip_connect = torch.tensor(skip_connect).to(heads.device)
    ...
    def decode(self, input_word, input_char, input_pos, mask=None, beam=1, leading_symbolic=0):
        ...
        for t in range(num_steps):
            ...
            # [batch, num_hyp]
            hyp_heads = curr_heads.gather(dim=1, index=base_index.long())
            hyp_gpars = curr_gpars.gather(dim=1, index=base_index.long())

            # [batch, num_hyp, length]
            base_index_expand = base_index.unsqueeze(2).expand(batch, num_hyp, max_len)
            constraints = constraints.gather(dim=1, index=base_index_expand.long())
            constraints.scatter_(2, child_index.unsqueeze(2), True)

            # [batch, num_hyp]
            mask_leaf = hyp_heads.eq(child_index)
            # [batch, num_hyp, length]
            heads = heads.gather(dim=1, index=base_index_expand.long())
            heads.scatter_(2, child_index.unsqueeze(2), torch.where(mask_leaf, hyp_gpars, hyp_heads).unsqueeze(2))
            types = types.gather(dim=1, index=base_index_expand.long())
            # [batch, num_hyp]
            org_types = types.gather(dim=2, index=child_index.unsqueeze(2).long()).squeeze(2)

            # [batch, num_hyp, num_steps]
            base_index_expand = base_index.unsqueeze(2).expand(batch, num_hyp, num_steps + 1)
            stacked_heads = stacked_heads.gather(dim=1, index=base_index_expand.long())
            stacked_heads[:, :, t + 1] = torch.where(mask_leaf, hyp_gpars, child_index)
            if self.sibling:
                siblings = siblings.gather(dim=1, index=base_index_expand.long())
            ...
            #print("num_htp, type_c, type_h: ", num_hyp, type_c.shape, type_h.shape)
            out_type = self.bilinear(type_h.gather(dim=1, index=base_index_expand.long()), type_c.gather(dim=1, index=child_index_expand.long()))
...
class StackPtrNet2(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, rnn_mode, hidden_size,
        ...
        super(StackPtrNet2, self).__init__()
        self.word_embed = nn.Embedding(num_words, word_dim, _weight=embedd_word, padding_idx=1)
        self.pos_embed = nn.Embedding(num_pos, pos_dim, _weight=embedd_pos, padding_idx=1) if pos else None
        self.char_embed = nn.Embedding(num_chars, char_dim, _weight=embedd_char, padding_idx=1)
        self.char_cnn = CharCNN(2, char_dim, char_dim, hidden_channels=char_dim * 4, activation=activation)
