...
from LossAttack.models.neuronlp2.io.common import DIGIT_RE
...
def load_embedding_dict(embedding, embedding_path, normalize_digits=True):
    ...
    if embedding == 'word2vec':
        ...
        barttkn = BartTokenizer.from_pretrained("facebook/bart-base")
        embd_weight = bart.get_input_embeddings().weight
        embedd_dim = embd_weight.shape[-1]
        embedd_dict = OrderedDict()
        for ind, embd in enumerate(embd_weight):
            w = barttkn._convert_id_to_token(ind)
            word = DIGIT_RE.sub("0", w) if normalize_digits else w
            embedd_dict[word] = embd.detach().numpy()
        return embedd_dict, embedd_dim
