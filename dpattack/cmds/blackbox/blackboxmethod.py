...
from dpattack.cmds.blackbox.attackindex import *
...
class Denoising(BlackBoxMethod):# validation of examples reconstructed by Bi-TIR
    def __init__(self, config, vocab, parser):
        ...
        self.vocab = vocab
        self.parser = parser
        with open("/data/luoyt/Transformer-master/result/gold_sort.txt",'r') as f:
            self.gold = f.readlines()
        with open("/data/luoyt/Transformer-master/result/res_sort.txt",'r') as f:
            self.pred = f.readlines()
        with open("/data/luoyt/Transformer-master/result/goldarc_sort.txt",'r') as f:
            self.goldarc = f.readlines()
        with open("/data/luoyt/Transformer-master/result/goldrel_sort.txt",'r') as f:
