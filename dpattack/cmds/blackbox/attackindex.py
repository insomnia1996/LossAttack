...
class AttackIndexUnkReplacement(AttackIndex):
    ...
    def get_loss_bak(self, s_arc,gold_arcs, s_rel,gold_rels):
        loss=[]
        ...
        for idx in range(s_arc.shape[0]):
            arc_loss = self.criterion(s_arc[idx], gold_arcs[0])
            rel_loss = self.criterion(logits_deprel[idx], gold_rels[0])
            loss.append(torch.log(arc_loss).item()+torch.log(rel_loss).item())#未mask前，整句话的loss都一样，因此mask后loss越大，说明mask前后loss差越大
        return loss

    def get_loss(self, s_arc,gold_arcs, s_rel,gold_rels):
        loss=[]
        
        for idx in range(s_arc.shape[0]):
            arc_loss = self.criterion(s_arc[idx], gold_arcs[0])
            loss.append(torch.log(arc_loss).item())
        return loss

    def get_attack_index_bak(self, seqs, seq_idx, tags, tag_idx, chars, arcs, rels, mask):#dpattack method
    ...
    def get_attack_index(self, seqs, seq_idx, tags, tag_idx, chars, arcs, rels, mask):#lossattack method
        ...
        #pred_arc = score_arc.argmax(dim=-1)#[seq_len, seq_len+num_punct], 每一种UNK替换后的句子标签输出，共seq_len个句子，seq_len+num_punct个标签以序列输出。
        pred_arc = score_arc#[:,mask.squeeze(0)]
        pred_rel = score_rel
        #print(arcs.shape,rels.shape)#[1, seq_len+num_punct]
        non_equal_numbers = self.get_loss(pred_arc[:,mask.squeeze(0)],arcs[:,mask.squeeze(0)], pred_rel[:,mask.squeeze(0)], rels[:,mask.squeeze(0)])#[seq_len]
        ...
        print("index_to_be_attacked: ", index_to_be_attacked)
    ...
    def get_index_to_be_attacked(self, non_equal_dict, number):
        ...
        for key in sorted(non_equal_dict.keys(), reverse=True):#优先改错误数目/loss最大的位置，再试下一个位置。
