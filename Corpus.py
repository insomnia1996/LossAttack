...
class Corpus(object):
    ...
    def pheads(self):
        final=[]
        for sentence in self:
            phead = list(sentence.PHEAD)
            for j, p in enumerate(phead):
                if p=='_':
                    phead[j] = int(sentence.HEAD[j])
                else:
                    phead[j] = int(sentence.PHEAD[j])
            final.append([0]+phead+[0])
        return final
    ...
    def prels(self):
        final=[]
        for sentence in self:
            pdeprel = list(sentence.PDEPREL)
            for j, p in enumerate(pdeprel):
                if p=='_':
                    pdeprel[j] = sentence.DEPREL[j]
                else:
                    pdeprel[j] = sentence.PDEPREL[j]
            final.append([self.ROOT] + pdeprel + [self.EOS])
        return final
    ...
    @pheads.setter
    def pheads(self, sequences):
        self.sentences = [sentence._replace(PHEAD=sequence)
                          for sentence, sequence in zip(self, sequences)]

    @prels.setter
    def prels(self, sequences):
