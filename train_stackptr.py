...
from LossAttack.models.neuronlp2.nn.utils import total_grad_norm
from LossAttack.models.neuronlp2.io import get_logger, conllx_data, conllx_stacked_data, iterate_data
from LossAttack.models.neuronlp2.models import StackPtrNet, StackPtrNet2
from LossAttack.models.neuronlp2.optim import ExponentialScheduler
from LossAttack.models.neuronlp2 import utils
from LossAttack.models.neuronlp2.io import CoNLLXWriter
from LossAttack.models.neuronlp2.tasks import parser
from LossAttack.models.neuronlp2.nn.utils import freeze_embedding
...
from LossAttack.models.neuronlp2.io.conllx_stacked_data import _generate_stack_inputs
...
def eval(alg, data, network,  punct_set, vocab, device, beam=1, batch_size=256):
    network.eval()
    accum_ucorr = 0.0
    accum_lcorr = 0.0
    accum_total = 0
    ...
    for words, postags, chars, heads, types in data:
        masks = words.ne(0)
        lengths = torch.sum(masks, dim=-1, keepdim=False)
        masks[:, 0] = 0
        ...
        stats, stats_nopunc, stats_root, num_inst = parser.eval(words, postags, heads_pred, types_pred, heads, types,
                                                                vocab, lengths, punct_set=punct_set, symbolic_root=True)
...
def train(args):
    ...
    train = Corpus.load(args.train)
    dev = Corpus.load(args.dev)
    test = Corpus.load(args.test)
    ...
    num_words = config.n_words# supposed to be 21680
    num_chars = config.n_chars#char_alphabet.size() # supposed to be 83
    num_pos = config.n_tags#pos_alphabet.size() # supposed to be 49
    num_types = config.n_rels#type_alphabet.size() # supposed to be 47
    ...
    def construct_word_embedding_table():
        ...
        table = np.empty([config.n_words, word_dim], dtype=np.float32)
        ...
        for word in vocab.word_dict:
            if word in word_dict:
                ...
                oov += 1
            table[vocab.word_dict[word], :] = embedding
    ...
    if model_type == 'StackPtr':
        ...
        network = StackPtrNet2(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, mode, hidden_size,
                              encoder_layers, decoder_layers, num_types, arc_space, type_space,
                              embedd_word=word_table, embedd_char=None, prior_order=prior_order, activation=activation,
    ...
    dev_loader = batchify(dataset=devset,
                            batch_size=args.batch_size,
                            n_buckets=64, collate_fn=collate_fn)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, data  in enumerate(train_loader):
            words, postags, chars, heads, types = data
            masks = words.ne(vocab.pad_index)
            nbatch = words.size(0)
            ...
            if alg == 'graph':
                ...
                for idx in range(heads.shape[0]): 
                    stacked_head, child, sibling, stacked_type, _ = _generate_stack_inputs(heads[idx], types[idx], 'inside_out')
                    stacked_heads.append(stacked_head)
                    children.append(child) 
                    siblings.append(sibling)
                    stacked_types.append(stacked_type)
                stacked_heads = torch.tensor(stacked_heads).to(heads.device)
                children = torch.tensor(children).to(heads.device)
                siblings = torch.tensor(siblings).to(heads.device)
                stacked_types = torch.tensor(stacked_types).to(heads.device)
        ...
        with torch.no_grad():
            ...
            dev_stats, dev_stats_nopunct, dev_stats_root = eval(alg, dev_loader, network, punct_set, vocab, device, beam=beam)
            ...
            if best_ucorrect_nopunc + best_lcorrect_nopunc < dev_ucorr_nopunc + dev_lcorr_nopunc:
                ...
                test_stats, test_stats_nopunct, test_stats_root = eval(alg, test_loader, network, punct_set, vocab, device, beam=beam)
...
def parse(args, words, chars, tags):#[batch_size, seq_len]
    logger = get_logger("Parsing")
    args.cuda = torch.cuda.is_available()
    device = torch.device('cuda', 0) if args.cuda else torch.device('cpu')
    ...
    if model_type == 'StackPtr':
        ...
        grandPar = hyps['grandPar']
        sibling = hyps['sibling']
        network = StackPtrNet2(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos,
    ...
    with torch.no_grad():
        ...
        postags = tags.to(device)
        if alg == 'graph':
            heads_pred, types_pred = network.decode(words, chars, postags, mask=None, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS)
        else:
            heads_pred, types_pred = network.decode(words, chars, postags, mask=None, beam=1, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS)
        print("head rel shape: ", heads_pred.shape, types_pred.shape)
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--word_embedding', choices=['glove', 'senna', 'sskip', 'polyglot', 'bart'], help='Embedding for words')
